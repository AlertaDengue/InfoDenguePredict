...
from infodenguepredict.data.infodengue import get_cluster_data, get_city_names, combined_data, \
    get_alerta_table, build_lagged_features
...
def rf_state_prediction(state, lookback, horizon, predictors, rerun=False):
    """
    ...
    for cluster in clusters:
        ...
        for city in cluster:
            if os.path.isfile('./saved_models/{}/rf_metrics_{}.pkl'.format(state, city)) and not rerun:
                print('done')
                continue
            ...
            metrics.to_pickle('{}/{}/rf_metrics_{}.pkl'.format('saved_models/random_forest', state, city))
            pickle.dump(preds, open('{}/{}/rf_prediction_{}.pkl'.format('saved_models/random_forest', state, city), 'wb'))
            plot_prediction(preds, targets[1], city_name, len(X_train))
            plt.show()
    return None
...
if __name__ == "__main__":
    ...
    rf_state_prediction(STATE,LOOK_BACK, PREDICTION_WINDOW, PREDICTORS, rerun=True)
